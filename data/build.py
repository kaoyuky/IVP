...
class UCMDataset(data.Dataset):
    def __init__(self, root, train=True, transform=None, split=None):
        ...
        train_infos.sort()
        trn_files = [os.path.join(root + '/all_img', eval(image_path)[0]) for image_path in train_infos]
        trn_targets = [eval(image_path)[1:] for image_path in train_infos]
        ...
        valid_infos.sort()
        val_files = [os.path.join(root + '/all_img', eval(image_path)[0]) for image_path in valid_infos]
        val_targets = [eval(image_path)[1:] for image_path in valid_infos]
    ...
    def __getitem__(self, i):
        ...
        return img, int(self.targets[i][0])
