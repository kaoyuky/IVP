...
class PromptGeneratorWithNaive(nn.Module):
    ...
    def forward(self, hidden_states=None, attention_mask=None):
        hidden_states = (self.avgpool(hidden_states.transpose(1, 2))).squeeze()#.unsqueeze(dim=0)
        ...
        hidden_states = hidden_states.view(hidden_states.shape[0], self.num_prompt_tokens, -1)

        return hidden_states


class PromptGeneratorWithPooling(nn.Module):
    def __init__(self, input: int, downsize: int, prompt: int, num):
        super().__init__()
    ...
    def forward(self, hidden_states=None, attention_mask=None):
        ...
        hidden_states = (self.adaptive_pooling(hidden_states))#.transpose(1, 2)
        hidden_states = hidden_states.transpose(1, 2)
...
class PromptedSwinTransformer(SwinTransformer):
    def __init__(
        self, cfg, img_size=224, patch_size=4, in_chans=3,
        ...
        self.prompt_LOCATION = cfg.LOCATION
        self.prompt_NUM_TOKENS = cfg.NUM_TOKENS
        ...
        if self.prompt_INITIATION == "random":
            ...
            self.prompt_embeddings = PromptGeneratorWithNaive(embed_dim, cfg.project, num_tokens)

            if self.prompt_DEEP:
                self.prompt_embeddings = PromptGeneratorWithPooling(embed_dim, cfg.project, num_tokens, (img_size[0]//patch_size[0])**2)
                self.deep_prompt_embeddings_0 = PromptGeneratorWithPooling(embed_dim, cfg.project, num_tokens, (img_size[0]//patch_size[0])**2)

                self.deep_prompt_embeddings_1 = nn.ModuleList()
                for i_layer in range(depths[1]):
                    layer = PromptGeneratorWithPooling(embed_dim*2, cfg.project, num_tokens, ((img_size[0]//patch_size[0])**2)//4)
                    self.deep_prompt_embeddings_1.append(layer)

                self.deep_prompt_embeddings_2 = nn.ModuleList()
                for i_layer in range(depths[2]):
                    layer = PromptGeneratorWithPooling(embed_dim*4, cfg.project, num_tokens, ((img_size[0]//patch_size[0])**2)//8)
                    self.deep_prompt_embeddings_2.append(layer)

                self.deep_prompt_embeddings_3 = nn.ModuleList()
                for i_layer in range(depths[3]):
                    layer = PromptGeneratorWithPooling(embed_dim * 8, cfg.project, num_tokens,
                                                                      ((img_size[0]//patch_size[0]) ** 2) // 16)
    ...
    def incorporate_prompt(self, x):
        ...
        if self.prompt_INITIATION == "random":
            ...
            x = self.get_patch_embeddings(x)
            prompt_embd = self.prompt_embeddings(x)
    ...
    def forward_features(self, x):
        ...
        if self.prompt_INITIATION == "random":
            ...
            for layer, net in zip(
                ...
                x = layer(x=x, deep_prompt_embd=None, net=net, droup=self.prompt_dropout)
