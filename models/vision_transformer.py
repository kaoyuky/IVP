...
class Block(nn.Module):
    ...
    def __init__(
        ...
        # self.num_tokens = 50
        # self.meta_net = PromptGeneratorWithNaivePromptGenerator(dim, 16, self.num_tokens)
        # self.prompt_dropout = nn.Dropout(0.1)

    def forward(self, x):
        # if x.shape[1] == 197:
        #     prompt_embeddings = self.meta_net(x[:, 1:, :])
        #     x = torch.cat((
        #         x[:, :1, :],
        #         self.prompt_dropout(
        #             prompt_embeddings),
        ...
        #     ), dim=1)
        # else:
        #     deep_prompt_embeddings = self.meta_net(x[:, (1 + self.num_tokens):, :])
        #     x = torch.cat((
        #         x[:, :1, :],
        #         self.prompt_dropout(
