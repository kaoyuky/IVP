...
class PromptedVisionTransformer(VisionTransformer):
    def __init__(self, prompt_config, **kwargs):
        ...
        if self.prompt_config.INITIATION == "random":
            self.meta_net_0 = PromptGeneratorWithNaivePromptGenerator(self.embed_dim, 16, self.num_tokens)
            self.meta_net_1 = PromptGeneratorWithNaivePromptGenerator(self.embed_dim, 16, self.num_tokens)
            self.meta_net_2 = PromptGeneratorWithNaivePromptGenerator(self.embed_dim, 16, self.num_tokens)
            self.meta_net_3 = PromptGeneratorWithNaivePromptGenerator(self.embed_dim, 16, self.num_tokens)
    ...
    def incorporate_prompt(self, x):
        ...
        if self.prompt_config.LOCATION == "prepend":
            ...
            prompt_embeddings = self.meta_net_0(x[:, 1:, :])
            x = torch.cat((
                    x[:, :1, :],
                    self.prompt_dropout(
                        prompt_embeddings),
    ...
    def forward_features(self, x):
        ...
        #         else:
        #             # prepend
        #             deep_prompt_embeddings = self.meta_net_0(x[:, (1 + self.num_tokens):, :])
        #             x = torch.cat((
        #                 x[:, :1, :],
        #                 self.prompt_dropout(
        ...
        if self.prompt_config.DEEP:
            ...
            for i in range(num_layers):
                if i == 0:
                    ...
                    deep_prompt_embeddings1 = self.meta_net_1(x[:, (1 + self.num_tokens):, :])
                    x = torch.cat((
                        x[:, :1, :],
                        self.prompt_dropout(
                            deep_prompt_embeddings1),
                        x[:, (1 + self.num_tokens):, :]
                    ), dim=1)
                    x = self.blocks[i](x)
                elif i>=6 and i<=8:
                    deep_prompt_embeddings2 = self.meta_net_2(x[:, (1 + self.num_tokens):, :])
                    x = torch.cat((
                        x[:, :1, :],
                        self.prompt_dropout(
                            deep_prompt_embeddings2),
                        x[:, (1 + self.num_tokens):, :]
                    ), dim=1)
                    x = self.blocks[i](x)
                else:
                    # prepend
                    deep_prompt_embeddings3 = self.meta_net_3(x[:, (1 + self.num_tokens):, :])
                    x = torch.cat((
                        x[:, :1, :],
                        self.prompt_dropout(
                            deep_prompt_embeddings3),
                        x[:, (1 + self.num_tokens):, :]
                    ), dim=1)
                    x = self.blocks[i](x)
        else:
