    ...
    for i in range(args.exp_num):
        ...
        for epoch in range(config.TRAIN.START_EPOCH, args.epochs):
            ...
            if acc1 > best_acc1:
                ...
                total_time_str1 = str(datetime.timedelta(seconds=int(time1)))
                logger.info('Onetime training time {}'.format(total_time_str1))
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            ...
            if config.AMP_OPT_LEVEL != "O0":
                ...
                if config.TRAIN.CLIP_GRAD:
                    ...
                    grad_norm = get_grad_norm(model.parameters())
            optimizer.step()
            # batch_time3.update(time.time() - y)
            lr_scheduler.step_update(epoch * num_steps + idx)
