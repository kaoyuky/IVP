...
class BasicLayer(nn.Module):
    def __init__(self, img_size=224, in_chans=3, embed_dims=64, token_dims=64, downsample_ratios=4, kernel_size=7, RC_heads=1, NC_heads=6, dilations=[1, 2, 3, 4],
                ...
                RC=None, NC=None, num_prompts=None, prompt_location=None, deep_prompt=None):
        super().__init__()
        self.img_size = img_size
        ...
        if RC_tokens_type == 'stem':
            ...
            if RC is not None:
                self.RC = RC(num_prompts, prompt_location, deep_prompt,
                             img_size, in_chans, embed_dims, token_dims, downsample_ratios, kernel_size,
                             RC_heads, dilations, op=RC_op, tokens_type=RC_tokens_type, group=RC_group, gamma=gamma,
                             init_values=init_values, SE=SE, relative_pos=relative_pos, window_size=window_size)
            else:
        ...
        if NC is not None:
            self.NC = nn.ModuleList([
                NC(num_prompts, prompt_location, deep_prompt,
                token_dims, NC_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
            ...
            self.NC = nn.ModuleList([
                NormalCell(token_dims, NC_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop,
                           ...
                           window_size=window_size, shift_size=0, relative_pos=relative_pos)
                for i in range(NC_depth)])

    def forward(self, x, NC_prompt_embd=None, prompt_dropout=None, INITIATION=None):
        if self.deep_prompt and NC_prompt_embd is None:
        ...
        if not self.deep_prompt:
            if len(x.shape) < 4:
                ...
                prompt = x[:, :self.num_prompts, :]
                prompt_copy = prompt
                prompt = torch.cat((prompt_copy, prompt_copy), dim=-1)
                p = False
            else:
                p = True
            x = self.RC(x)
            ...
            if p is True:
                if INITIATION=="random":
                    prompt_emb = prompt_dropout(NC_prompt_embd.expand(B, -1, -1))
                else:
                    prompt_emb = prompt_dropout(NC_prompt_embd(x))
                x = torch.cat(
                    (prompt_emb, x),
                    dim=1
                )
            ...
            num_blocks = len(self.NC)
            B = x.shape[0]
            if len(x.shape) < 4:
                # B,N,C -> B,H,W,C
                x = x[:, self.num_prompts:, :]
            x = self.RC(x)#, RC_prompt_embd1.expand(B, -1, -1))
            for i in range(num_blocks):
                if i == 0:
                    if INITIATION == "random":
                        prompt_emb = prompt_dropout(NC_prompt_embd[i].expand(B, -1, -1))
                    else:
                        prompt_emb = prompt_dropout(NC_prompt_embd[i](x))
                    x = torch.cat(
                        (prompt_emb, x),
                        dim=1
                    )
                    ...
                    if INITIATION == "random":
                        ...
                        prompt_emb = prompt_dropout(NC_prompt_embd[i](x[:, self.num_prompts:, :]))
                    x = torch.cat(
